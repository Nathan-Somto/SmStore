    ...
    <footer className="bg-[#252525] text-[#ebebeb] py-[3.38rem] px-[4rem] relative">
       <div className="flex flex-col space-y-4 lg:space-x-[25%] lg:space-y-0   lg:flex-row ">
        <div className="flex space-x-24 max-[500px]:flex-col max-[500px]:space-x-0 max-[500px]:space-y-4 lg:space-x-[25%]">
          ...
          <div className="flex space-x-4">
            <Link to={'/category/acessories'} className="opacity-70 hover:opacity-100 transition-all ease-in duration-300 hover:text-white">Categories</Link>
            <Link to={'/about'} className="opacity-70 hover:opacity-100 transition-all ease-in duration-300 hover:text-white">About</Link>
            <Link to={'/contact'} className="opacity-70 hover:opacity-100 transition-all ease-in duration-300 hover:text-white">Contact</Link>
          </div>
        </div>
       ...
       <div className="flex flex-col space-y-8 lg:space-y-0 lg:flex-row lg:space-x-[25%]  mt-8">
        <form className="relative">
          ...
          <input className=' border-b-2 border-solid border-[#939292] bg-transparent w-full text-sm pb-2 font-normal placeholder:text-[#a4a4a4]' type="email" id="email"  name="email" placeholder="Your email here" />
          <button className="absolute right-[0] bottom-2"><HiArrowRight/></button>
          </label>
        </form>
        <div className="flex space-x-4 items-end ">
          <Link to={'https://www.facebook.com'} className="hover:scale-125 transition-all ease-out duration-300">
            <FaFacebook/>
          </Link>
          <Link to={'https://www.instagram.com'} className="hover:scale-125 transition-all ease-out duration-300">
            <FaInstagram/>
          </Link>
          <Link to={'https://www.twitter.com'} className="hover:scale-125 transition-all ease-out duration-300">
            <FaTwitter/>
          </Link>
          <Link to={'https://www.pinterest.com'} className="hover:scale-125 transition-all ease-out duration-300">
            <FaPinterest/>
          </Link>
          <Link to={'https://www.youtube.com'} className="hover:scale-125 transition-all ease-out duration-300">
            <FaYoutube/>
          </Link>
        </div>
       </div>
       <div className="absolute bottom-0 px-[4rem] left-0 text-center w-full bg-[#6b6b6b]">
        <p>Coded by <Link to={'https://www.github.com/nathan-somto'} className="mx-2 text-slate-900 font-semibold">Nathan Somto</Link>Copyright © {new Date().getFullYear()}</p>
       </div>
    </footer>
  )
}
